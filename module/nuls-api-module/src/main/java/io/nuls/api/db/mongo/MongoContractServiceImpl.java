...
public class MongoContractServiceImpl implements ContractService {
    ...
    public PageInfo<MiniContractInfo> getContractList(int chainId, int pageNumber, int pageSize, boolean onlyNrc20, int tokenType, boolean isHidden) {
        Bson filter = null;
        if (onlyNrc20) {
            if (tokenType == -1) {
                filter = Filters.eq("isNrc20", true);
            } else {
                filter = Filters.and(Filters.eq("isNrc20", true), Filters.eq("tokenType", tokenType));
            }
        } else if (isHidden) {
    ...
    @Override
    public PageInfo<MiniContractInfo> getContractList(int chainId, int pageNumber, int pageSize, String address, boolean onlyNrc20, int tokenType, boolean isHidden) {
        Bson filter = null;
        if (onlyNrc20) {
            if (tokenType == -1) {
                filter = Filters.and(Filters.eq("isNrc20", true), Filters.eq("creater", address));
            } else {
                filter = Filters.and(Filters.eq("isNrc20", true), Filters.eq("creater", address), Filters.eq("tokenType", tokenType));
            }
        } else if (isHidden) {
