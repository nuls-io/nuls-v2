...
public class CompletedCtxServiceImpl implements CompletedCtxService {
    ...
    public boolean save(byte[] atxHash, Transaction ctx, int chainID) {
        try {
            ...
            return RocksDBService.put(NulsCrossChainConstant.DB_NAME_COMPLETED_CTX+chainID,atxHash,ctx.serialize());
        }catch (Exception e){
            Log.error(e);
        }
        return false;
    ...
    public Transaction get(byte[] atxHash, int chainID) {
        try {
            ...
            byte[] txBytes = RocksDBService.get(NulsCrossChainConstant.DB_NAME_COMPLETED_CTX+chainID,atxHash);
            if(txBytes == null){
                return null;
            }
    ...
    public boolean delete(byte[] atxHash, int chainID) {
        try {
            ...
            return RocksDBService.delete(NulsCrossChainConstant.DB_NAME_COMPLETED_CTX+chainID,atxHash);
        }catch (Exception e){
            Log.error(e);
        }
        return false;
