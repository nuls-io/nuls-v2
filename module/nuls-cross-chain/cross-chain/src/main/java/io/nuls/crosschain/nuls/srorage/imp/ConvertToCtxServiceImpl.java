...
public class ConvertToCtxServiceImpl implements ConvertToCtxService {
    ...
    public boolean save(byte[] originalHash, byte[] localHash, int chainID) {
        ...
        try {
            return RocksDBService.put(NulsCrossChainConstant.DB_NAME_CONVERT_TO_CTX + chainID, originalHash, localHash);
        } catch (Exception e) {
            Log.error(e);
        }
        return false;
    ...
    public byte[] get(byte[] originalHash, int chainID) {
        ...
        try {
            byte[] valueBytes = RocksDBService.get(NulsCrossChainConstant.DB_NAME_CONVERT_TO_CTX + chainID, originalHash);
            return valueBytes;
        } catch (Exception e) {
            Log.error(e);
        }
    ...
    public boolean delete(byte[] originalHash, int chainID) {
        try {
            ...
            return RocksDBService.delete(NulsCrossChainConstant.DB_NAME_CONVERT_TO_CTX + chainID, originalHash);
        } catch (Exception e) {
            Log.error(e);
        }
        return false;
